...
const ListItem = ({ data }: Props) => (
  <Link href="/users/[id]" as={`/users/${data.id}`}>
    <div className="max-w-sm bg-white rounded-lg shadow-md dark:bg-gray-800 dark:border-gray-700 m-4">
      <a href="#">
        <img className="p-8 rounded-t-lg object-cover h-72 w-96" src={`https://thirosue.github.io/amplify-admin-ui-sample/static/${data.id % 4 + 1}.jpg`} alt="product image" />
      </a>
      <div className="px-5 pb-5">
        <a href="#">
          <h5 className="text-xl font-semibold tracking-tight text-gray-900 dark:text-white">{data.id} {data.name}</h5>
        </a>
        ...
        <div className="flex justify-between items-center">
          <span className="text-3xl font-bold text-gray-900 dark:text-white">${data.id}</span>
